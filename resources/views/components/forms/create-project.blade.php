<div class="w-full sm:max-w-md px-6 py-4">
    <form wire:submit.prevent="save">
        ...
        <div class="mt-4">
            ...
            <x-inputs.img wire:model="imageFile" id="projectImage">
                <span class="w-24 rounded-lg overflow-hidden bg-gray-100">
                    <img src="{{ $imageFile ? $imageFile->temporaryUrl() : $currentProject->image_url }}"
                        alt="{{ __('Project image') }}">
                </span>
            </x-inputs.img>

            <div wire:loading wire:target="imageFile" class="mt-1 w-full text-emerald-700">
                {{ __('Verifying image...') }}
            </div>
        ...
        <div class="mt-4">
            ...
            @error('currentProject.video_link')
                <div class="mt-1 text-red-600 text-sm">{{ $message }}</div>
            @enderror
        </div>

        <div class="mt-3">
            <x-inputs.label for="url" value="{{__('Web page path')}} {{ __('(URL)') }}" />

            <x-inputs.text wire:model.defer="currentProject.url" id="url" type="text" />

            @error('currentProject.url')
                <div class="mt-1 text-red-600 text-sm">{{ $message }}</div>
            @enderror
        </div>
